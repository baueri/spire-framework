...
use Baueri\Spire\Framework\Database\Repository\Events\EntityCreated;
use Baueri\Spire\Framework\Database\Repository\Events\EntityDeleted;
use Baueri\Spire\Framework\Event\EventDispatcher;
use Baueri\Spire\Framework\Entity\Exceptions\EntityNotFoundException;
use Baueri\Spire\Framework\Entity\Exceptions\QueryBuilderException;
use Baueri\Spire\Framework\Entity\Relation\HasRelations;
...
class EntityQueryBuilder
{
    ...
    /**
     * @phpstan-param class-string<T>|null $entity
     */
    final public function __construct(?string $entity = null)
    {
    ...
    public function getTableName(): ?string
    {
        ...
        if ($this->entityClass) {
            return Inflector::tableize(get_class_name($this->entityClass));
        }
    ...
    /**
     * @phpstan-return T[]|EntityCollection<T>
     */
    public function get(): EntityCollection
    {
        return $this->loadRelations(
            new EntityCollection(array_map(function ($row) {
    ...
    public function deleteModel(int|string|Entity $model, bool $hardDelete = false): bool
    {
        ...
        EventDispatcher::dispatch(new EntityDeleted($model));
    ...
    public function paginate(?int $perpage = null, ?int $page = null): PaginatedEntityCollection
    {
        $rows = $this->builder->paginateRaw($perpage, $page);
        $models = new PaginatedEntityCollection(array_map(function ($row) {
            return $this->getInstance($row);
    ...
    public function create(array|Entity $values): Entity|null
    {
        ...
        EventDispatcher::dispatch(new EntityCreated($model));
